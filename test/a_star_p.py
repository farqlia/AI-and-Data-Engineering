...
with open(str(A_STAR_FILE) + 'ChangeHeuristic', mode='w', encoding='utf-8') as f:
    for (start_stop, goal_stop, leave_hour) in test_cases:
        ...
        goal, came_from_conn, stop_conn, cost_so_far = find_path(g, ChangeHeuristic(), start_stop, goal_stop, leave_hour)
        end = timer()
        ...
        conns.reverse()
        print(conns)
        print_path([g.conn_at_index(idx) for idx in conns], print_to=f)
        print(round(cost_so_far[goal], 2), file=f)
